...
def parse_pairs(dict_text):
    """Parse key-value pairs from dict text, handling multiline strings."""
    pairs = []
    i = 0
    depth = 0
    in_string = False
    ...
    for i, ch in enumerate(dict_text):
        if in_string:
            ...
            if ch in '"\'':
                ...
                part = dict_text[part_start:i].strip()
                if part:
                    pairs.append(part)
                part_start = i + 1
    part = dict_text[part_start:].strip()
    if part:
        pairs.append(part)
    return pairs

def parse_kv(part):
    """Parse key and value from a "key": value pair."""
    colon_idx = -1
    in_str = False
    sc = None
...
en_text = get_dict_text(r'V:\___VAC\__K\__CODE\_PY\_FastPrompter\src\fastprompter\core\i18n\en.py')
en_pairs = parse_pairs(en_text)
en_dict = {}
for p in en_pairs:
    k, v = parse_kv(p)
    if k:
...
# Parse existing Ukrainian
ukr_text = get_dict_text(r'V:\___VAC\__K\__CODE\_PY\_FastPrompter\src\fastprompter\core\i18n\ukr.py')
ukr_pairs = parse_pairs(ukr_text)
ukr_dict = {}
for p in ukr_pairs:
    k, v = parse_kv(p)
    if k:
...
# Missing keys
missing = [k for k in en_dict if k not in ukr_dict]
print(f"Missing: {len(missing)}")

# Ukrainian translations for the missing keys
new_translations = {
    ...
    '50&ndash;150% whole-UI scaling with readable minimums': 'Масштабування всього інтерфейсу 50&ndash;150% з читабельними мінімумами',
    ...
    'Copy that code block to the clipboard': 'Копіювати блок коду в буфер обміну',
    'Create these folders in the current silo': 'Створити ці папки в поточному силі',
    'Creates an exact copy of the local_data_v15.db file containing all settings, silos, and snippets.': 'Створює точну копію файлу local_data_v15.db з усіма налаштуваннями, сілами та снипетами.',
    ...
    'Ctrl+H : Replace Text': 'Ctrl+H : Замінити текст',
    ...
    'Drop files here \u2014 copied into a plain folder you own. Hold Alt while dropping to add links instead of copies.': 'Кидайте файли сюди — копіюються у звичайну папку. Утримуйте Alt, щоб додати посилання замість копій.',
    ...
    'Export All...\nCopy every file here to a folder you pick': 'Експорт All...\nСкопіювати всі файли в обрану папку',
    'Export Silos & Text': 'Експорт сіл та тексту',
    'Export all Silo contents to readable text formats.': 'Експортувати всі сили у читабельні текстові формати.',
    ...
    'Font loaded but no font families found.': 'Шрифт завантажено, але сімейства шрифтів не знайдено.',
    'Format:': 'Формат:',
    'Header the line: # + bold + underline + timestamp, then jump 2 lines down onto a fresh &bull; bullet': 'Оформити рядок: # + жирний + підкреслений + час, потім перейти на 2 рядки вниз на новий &bull; маркер',
    "How should '{}' be added?": "Як додати '{}'?",
    "Import Files...\nCopy files into this silo's folder\n(or just drop files anywhere on this window)": "Імпорт файлів...\nСкопіювати файли в папку цього сила\n(або просто перетягніть файли на це вікно)",
    ...
    'Loaded: {}': 'Завантажено: {}',
    'Lock / unlock window size & position': 'Заблокувати / розблокувати розмір та положення вікна',
    'Mark it done \u2014 the tick stays until clicked again': 'Позначити як виконане — позначка залишається до повторного кліку',
    'Markdown Files (*.md)': 'Markdown файли (*.md)',
    'Maximum of 5 tabs/projects. Remove one first.': 'Максимум 5 вкладок/проектів. Спочатку видаліть одну.',
    ...
    'Name:': 'Ім\'я:',
    'Nest it as a child (1 level; its files can merge into the parent)': 'Вкласти як дочірнє (1 рівень; файли можуть об\'єднатися з батьком)',
    'New Folder\tCtrl+N': 'Нова папка\tCtrl+N',
    'New empty silo at the top (max 5 blanks)': 'Нове порожнє сило зверху (макс. 5 порожніх)',
    ...
    'Reorder \u2014 dragging a child out promotes it back to top level': 'Перевпорядкування — перетягування дочірнього піднімає його на верхній рівень',
    'Replace with...': 'Замінити на...',
    'Replaced {} occurrences.': 'Замінено {} входжень.',
    'SQLite next to the app; daily Markdown backups in Documents; crash log next to the EXE': 'SQLite поруч з додатком; щоденні Markdown резервні копії в Documents; лог помилок поруч з EXE',
    'Save && Apply': 'Зберегти && Застосувати',
    'Save Clipboard': 'Зберегти буфер обміну',
    'Save Snippet': 'Зберегти снипет',
    'Save current silo as file in its own folder:': 'Зберегти поточне сило як файл у його папці:',
    'Save text as snippet / update the edited snippet': 'Зберегти текст як снипет / оновити редагований снипет',
    'Select Export Directory': 'Виберіть каталог експорту',
    'Select previous / next silo': 'Вибрати попереднє / наступне сило',
    'Settings &rarr; Header Fmt: {text}, {time}, {state} (Morning/Day/Evening/Night) \u2014 bold markers are yours to keep or drop': 'Налаштування &rarr; Формат заголовка: {text}, {time}, {state} (Ранок/День/Вечір/Ніч) \u2014 маркдери можна залишити або прибрати',
    'Settings &rarr; Header Fmt: {{text}}, {{time}}, {{state}} (Morning/Day/Evening/Night) \u2014 bold markers are yours to keep or drop': 'Налаштування &rarr; Формат заголовка: {{text}}, {{time}}, {{state}} (Ранок/День/Вечір/Ніч) \u2014 маркдери можна залишити або прибрати',
    'Show / hide FastPrompter from anywhere': 'Показати / сховати FastPrompter звідки завгодно',
    'Show / hide the line-number gutter\n(click the gutter to place colored margin marks)': 'Показати / сховати нумерацію рядків\n(клікніть на поля для кольорових позначок)',
    'Show window + toggle the sidebar': 'Показати вікно + перемкнути бічну панель',
    ...
    'Transfer to project, replace from, move to bottom&hellip;': 'Перенести в проект, замінити з, перемістити вниз&hellip;',
    'UI Gaps:': 'Проміжки UI:',
    'Undo / redo \u2014 text <i>and</i> silo actions (clear, delete, move, pin, archive, tabs)': 'Скасувати / повторити — текст <i>та</i> дії з сила (очистити, видалити, перемістити, pin, архів, вкладки)',
    'View\nCycle view: Icons \u2192 List \u2192 Details (like Explorer)': 'Вид\nЦикл: Значки \u2192 Список \u2192 Деталі (як у провіднику)',
    'View ({})': 'Вид ({})',
    'View ({})\nCycle view: Icons \u2192 List \u2192 Details (like Explorer)': 'Вид ({})\nЦикл: Значки \u2192 Список \u2192 Деталі (як у провіднику)',
    'Zen / focus mode (hide all chrome)': 'Дзен / режим фокусу (сховати всі елементи)',
    'Zoom the editor font': 'Масштаб шрифту редактора',
    '``` fences render monospace with syntax tints, auto line numbers and a one-click copy button on the fence line': '``` блоки рендеряться моноширинно з підсвіткою, номерами рядків та кнопкою копіювання',
    'add shortcut in container': 'додати ярлик у контейнер',
    'clearing or trashing a silo writes its text to data/files/_trash/ and moves its files there; nothing is destroyed': 'очищення або видалення сила записує текст у data/files/_trash/ та переміщує файли туди; нічого не знищується',
    'clipboard has no text': 'буфер обміну порожній',
    'collapse code blocks and # header sections with the fold box; right-click &rarr; Expand All Folds': 'згортайте блоки коду та # заголовки через бокс згортання; правий клік &rarr; Розгорнути все',
    'date + time with seconds, day word and an optional mini analog clock, all toggleable': 'дата + час з секундами, словом доби та опціональним аналоговим годинником',
    'insert content into silo': 'вставити вміст у сило',
    'insert markdown link at cursor': 'вставити markdown посилання біля курсора',
    'live highlighting, clickable links &amp; checkboxes, auto-bullets (- + space, Enter continues), zebra stripes, line numbers': 'жива підсвітка, клікабельні посилання та прапорці, авто-буліти, зебра, номери рядків',
    'named text blocks per project tab; instant paste': 'іменовані текстові блоки на вкладку; миттєва вставка',
    'one click stores the current silo or snippet': 'один клік зберігає поточне сило або снипет',
    'optional UI clicks and typewriter effect': 'опціональні звуки кліків та друкарської машинки',
    'path copied': 'шлях скопійовано',
    'per-silo asset drawer: drop ANY files in, drag them out, preview images, open, export, link (.url), save clipboard as file. Explorer-style Icons / List / Details views': 'сховище файлів для сила: кидайте БУДЬ-ЯКІ файли, перетягуйте назовні, переглядайте, відкривайте, експортуйте, лінкуйте (.url), зберігайте буфер як файл. Вид: Значки / Список / Деталі',
    "store in silo's container": "зберегти в контейнері сила",
    'up to 100 auto-saved scratchpads per project; pins, recency color tints, line counters, drag to reorder': 'до 100 авто-збережених чернеток на проект; кріплення, кольори давності, лічильники рядків, перетягування',
    'up to 5 tabs, each with its own silos, snippets, archive': 'до 5 вкладок, кожна зі своїми сілами, снипетами та архівом',
    '{} file(s)': '{} файл(ів)',
    '{} item(s) \u00b7 {}': '{} елемент(ів) \u00b7 {}',
    '~50 text formats load as plain text': '~50 текстових форматів відкриваються як звичайний текст',
    '\U0001f4c1{}': '\U0001f4c1{}',
...
# Verify all missing keys are covered
still_missing = [k for k in missing if k not in new_translations]
with open('still_missing.txt', 'w', encoding='utf-8') as f:
    if still_missing:
        ...
        f.write("All 198 missing keys have translations!\n")

# Build the complete dictionary
complete = {}
complete.update(ukr_dict)  # Existing translations
for k in new_translations:
    complete[k] = new_translations[k]  # New translations

# Verify all EN keys are present
for k in en_dict:
    if k not in complete:
        print(f"Missing from complete: {repr(k)}")

# Sort and write
sorted_keys = sorted(complete.keys(), key=lambda x: x.lower())

output = []
output.append('"""Українські переклади (Ukrainian) — всі ключі."""')
output.append('')
output.append('from __future__ import annotations')
output.append('')
output.append('TRANSLATIONS: dict[str, str] = {')

for k in sorted_keys:
    v = complete[k]
    # Escape backslashes, quotes, and newlines for Python string representation
    # Use repr for proper escaping
    output.append(f'    {repr(k)}: {repr(v)},')
...
print(f"\nWritten {len(complete)} keys to ukr_full.py")
