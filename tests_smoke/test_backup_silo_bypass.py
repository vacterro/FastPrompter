...
which (a) used the RAW category name Ч ``..\\outside`` escapes the files
root entirely Ч (b) accepted ANY user filename, so ``..\\evil.txt`` or
...
def test_hostile_category_name_still_resolves_inside_the_root(hostile_win):
    ...
    inside the files root Ч never use the raw name as a path segment."""
    win, root = hostile_win
