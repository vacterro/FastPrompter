...
class FastPrompterState:
    ...
    def init_db(self):
        try:
            ...
            for row in cur.execute('SELECT key, value FROM settings'):
                ...
                elif row[0] in ('silo_last_edited_all', 'pinned_silos_all'):
    ...
    def _snapshot_state(self):
        ...
        self._last_saved_settings = {k: (json.dumps(v) if k in ("cats_order", "custom_colors", "silo_last_edited", "pinned_silos", "silo_last_edited_all", "pinned_silos_all") else str(v)) for k, v in self.data.items() if k not in ("categories", "temp_presets_all", "archive_temp_presets_all", "temp_presets", "archive_temp_presets")}
    ...
    def _save_data_to_db_locked(self, current_text, ui_settings=None, force=False):
        ...
        try:
            ...
            current_settings = {k: (json.dumps(v) if k in ("cats_order", "custom_colors", "silo_last_edited", "pinned_silos", "silo_last_edited_all", "pinned_silos_all") else str(v)) for k, v in self.data.items() if k not in ("categories", "temp_presets_all", "archive_temp_presets_all", "temp_presets", "archive_temp_presets")}
