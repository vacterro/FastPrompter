...
class CellWidget(QLineEdit):
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key.Key_Tab:
            ...
            if r == len(grid.cells) - 1 and c == len(grid.cells[0]) - 1:
                grid.insert_row(self, 1)
...
class TableGridWidget(QWidget):
    ...
    def _add_action_buttons(self):
        ...
        self.btn_add_col.setStyleSheet("QLabel { color: gray; padding: 2px; } QLabel:hover { color: white; }")
        self.btn_add_col.mousePressEvent = lambda e: self.insert_col(self.cells[0][-1], 1)
        self.layout.addWidget(self.btn_add_col, 0, len(self.cells[0]))
        
        self.btn_add_row = QLabel("➕ Add Row")
        self.btn_add_row.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_add_row.setStyleSheet("QLabel { color: gray; padding: 4px; } QLabel:hover { color: white; }")
        self.btn_add_row.mousePressEvent = lambda e: self.insert_row(self.cells[-1][0], 1)
        self.layout.addWidget(self.btn_add_row, len(self.cells), 0, 1, len(self.cells[0]))
