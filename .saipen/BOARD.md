...
| T-556 | DONE | - | Hashtags (minimal): #tag highlighted, Ctrl+click finds it across every silo, double-click jumps to the line. No tag store, no rename, no manager - a tag exists only while written. files: core/hashtags.py, ui/hashtag_dialog.py, ui/markdown_highlighter.py, ui/editor.py, main.py (open_hashtag_dialog, jump_to_silo_line), tests/test_hashtags.py, tests_smoke. (verified: 560 unit + 185 smoke PASS, conf: high) |
| T-557 | DONE | - | Custom cursors: use the user's own Windows scheme inside the app (default off), with a "Set in system" button (confirmed) whose right-click opens the full set. files: ui/cursor_theme.py, main.py, ui/editor.py, tests/test_cursor_theme.py, tests_smoke. (verified: 568 unit + 186 smoke PASS, conf: high) |
| T-558 | DONE | - | Tests were writing to the app's own log file, burying and eventually rotating away real crash evidence. Under pytest the log is now fastprompter-tests.log. files: core/logging.py, tests/test_logging.py. (verified: same run, conf: high) |
| T-555 | TODO | - | Bug (user): view jumps to the top, MORE OFTEN ON CLICK, "like a mini crash". Still not reproduced. Instrumented instead: mousePressEvent now logs a full traceback (was debug-level str(e)), and _watch_scroll_reset logs a call stack whenever the scrollbar slams from >200 to 0. Next occurrence should leave evidence in %TEMP%/fastprompter.log. |
| T-557 | DONE (reworked) | - | Custom cursors: the program keeps its OWN COPY of the set (data_dir/cursors + manifest), not a mirror of the live registry - otherwise the toggle changed nothing visible. Reads saved schemes too, so the set can be grabbed while Windows is on stock cursors. files: ui/cursor_theme.py, main.py, tests/test_cursor_theme.py, tests_smoke. (verified: 577 unit + 186 smoke PASS, conf: high) |
| T-559 | DONE | T-557 | Bug (user): custom cursors not applied on restart until the toggle was flipped. A pre-ticked checkbox does not fire its callback and nothing applied them at startup. files: main.py, tests_smoke. (verified: 577 unit + 187 smoke PASS, regression test confirmed by reverting the fix, conf: high) |
| T-560 | DONE | - | Ctrl+Q picker is now a compact map under the cursor (was full-screen), exactly two Tab pages (Quarters / Columns 640-800-640), last page remembered. Custom NxM grid + its Settings spins removed as orphaned. files: ui/fancy_zones.py, main.py, tests_smoke. (verified: 577 unit + 190 smoke PASS, conf: high) |
| T-561 | DONE | T-560 | Snapping no longer hides a window set to hide-on-click-out: the picker holds the dialogs' focus lock for its lifetime and releases it after the window is back. files: ui/fancy_zones.py, tests_smoke. (verified: same run, conf: high) |
| T-562 | TODO | - | Uncommitted third-party work was found in the tree and committed as aa70c09 at the user's request (Ctrl+E centering, text-align combo, FlowLayout justification, Tab focus, silo page buttons). NOT authored here and NOT reviewed line by line - both suites were green, but it deserves a proper review pass before it is relied on. |
| T-562 | DONE | - | Proper review of aa70c09 (third-party work). Three findings, all reproduced with measurements; P0 and P1 fixed here, see T-563..T-565. |
| T-563 | DONE | - | P0 (mine): 593af5a patched toggle_header_line, which nothing in the app calls - Ctrl+E and the H button both go to apply_header_timestamp, so the promised header reversal never reached the user. Fixed at the real call site, timestamp/centring/bold stripped too. files: main.py, tests_smoke. (verified: 577 unit + 193 smoke PASS, conf: high) |
| T-564 | DONE | T-562 | P2: ctrl_e_center defaulted to True (silent behaviour change) and does not survive a reload. Default is now False and the tooltip says it is visual only. files: main.py, tests_smoke. |
| T-565 | DONE | T-562 | P1-visual: FlowLayout justified every line, measured a 724px gap between two Clock-tab checkboxes - the exact "huge empty space" the panel was compacted to remove. Left-packing restored; gaps are a uniform 8px. files: ui/flow_layout.py, tests_smoke. |
| T-566 | TODO | - | toggle_header_line() in formatting_mixin.py is dead code: only tests call it. Either wire it to something or delete it with its tests - leaving two header functions with different behaviour is how T-563 happened. |
| T-567 | DONE | - | Bug (user): opaque square behind the analog clock. It painted the Default theme's toolbar tint on every theme; it now fills its own rect with the live tint, and header_tint() has a single owner in theme/themes.py. files: ui/analog_clock.py, theme/themes.py, ui/theme_mixin.py, tests_smoke. (verified: 577 unit + 195 smoke PASS across all 9 themes, conf: high) |
| T-566 | BLOCKED | T-569 | Delete toggle_header_line. The deletion is correct in itself, but it changes the fuzz test's RNG sequence and the header tests downstream fail. Blocked until the suite is stable enough to tell a real regression from fixture leakage. |
| T-568 | BLOCKED | T-569 | Bug (user): icons squashed/vanish on theme change. ROOT CAUSE FOUND: _apply_header_density applies visibility, labels and sizes only when the tier FLIPS, so the header's look depends on the history of flips, not the width; a theme change re-triggers a flip. Fix attempted and REVERTED after two failed cycles - dense/ultra lists overlap (second loop undid the first) and apply_button_size() also resets the button font, inflating the header 960 -> 1047px. Needs a focused pass on a stable suite. |
| T-569 | P0 | - | tests_smoke is RED at HEAD (5 failed / 190 passed) on a clean tree, at a commit that measured 195 passed an hour earlier. Not isolated. Ruled out: the toggle_header_line deletion, the leftover data/cursors bundle, the second-window global-style leak in the restart test. Note: running the five alone via -k fails at EVERY commit including known-green ones, so isolation runs are not a valid signal for them. Suspect T-520 (one shared win fixture across 195 tests). Fix this before trusting any header/theme work. |
| T-567 | DONE (redo) | - | The clock filled its rect with the header BAR tint, but its neighbours paint bg_main - a pale square among dark widgets. Fills bg_main now; verified against the real neighbours on all 9 themes. files: ui/analog_clock.py, tests_smoke. (verified: 577 unit; smoke adds no new failure beyond the 5 in T-569, conf: high) |
| T-570 | P1 | T-569 | A full smoke run SEGFAULTED once (Qt crash, no Python traceback) while a test grabbed the clock repeatedly across theme switches. Hard evidence that the suite instability in T-569 is a real crash, not just fixture leakage. Same family as the documented setExtraSelections re-entrancy. |
| T-567 | DONE (final) | - | The square was the header tint showing in the GAP around an 18x18 clock in a 26px row - outside the widget, so no fill could hide it. The clock is now 18 x row-height with the dial centred. files: ui/analog_clock.py. (verified: pixel map; header height and sizeHint unchanged, conf: high) |
| T-568 | BLOCKED (needs user data) | - | Icon/text squashing not reproducible offscreen at any theme, width (1100..460) or scale (1.0/0.75/0.5). Lead: buttons report pointSizeF = -1, i.e. the theme stylesheet sets the font size in PIXELS while _scale_button_font sets POINTS - the two fight and a theme change decides which wins. Also button widths never shrink with ui_scale (49/60/35 at every scale) while heights do (24 -> 20). Needs from the user: exact Scale %, window width, and whether it shows immediately on the theme switch or only after a resize. |
| T-571 | DONE | - | Header layout spacing set to 0: the bar's tint showed as a 1px line in all 31 gaps between widgets. Verified 31/31 clean afterwards. files: main.py. (verified: 577 unit green; smoke shows the same 5 known failures, none new, conf: high) |
| T-570 | P1 (sharpened) | T-569 | The segfault happens at TEARDOWN, after the progress line reaches 100%. That is why the final "N passed" line is sometimes missing and why the same commit appeared to score 195 and then 190 - the tally may simply never have been printed. Count the F marks in the progress line, not the summary. Fix the crash before trusting any pass count. |
