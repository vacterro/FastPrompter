...
| Protokolldatei | Pfad | Zweck |
|---|---|---|
| **Anwendungsprotokoll** | `%TEMP%\fastprompter.log` | Aktive Sitzungsereignisse, Warnungen, Infoprotokolle (rotierend, max. 1 MB, 2 Backups). |
| **Absturzprotokoll** | `%TEMP%\fastprompter_crash.log` | Detaillierte Rückverfolgung nicht behandelter kritischer Ausnahmen über „sys.exclusivehook“. |
| **Testsuite-Protokoll** | `%TEMP%\fastprompter-tests.log` | Temporäres Protokoll, isoliert auf Pytest-Ausführungsläufe. |
