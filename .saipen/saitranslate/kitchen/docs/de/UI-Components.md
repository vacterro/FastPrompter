...
| Dialog | Zweck |
|---|---|
| `SettingsDialog` (`ui/settings.py`) | Themenauswahl, Hotkey-Rebuilder, Soundsteuerung, UI-Skalierungsschieberegler |
| `SaipenDialog` (`ui/saipen_dialog.py`) | Spezieller Viewer für „.saipen“-Projektstatus, Taskboard und Ereignisprotokolle |
| `TimerDialog` (`ui/timer_dialog.py`) | Einrichtung des Pomodoro-Fokus-Timers, Optimierung der Dauer und akustische Alarme |
| `TrashDialog` (`ui/trash_dialog.py`) | Papierkorb-Browser zum Anzeigen und Wiederherstellen vorläufig gelöschter Silos |
| `BackupDialog` (`ui/backup_dialog.py`) | Vollständiger Datenbankexport, -import und Erstellung von Backup-Snapshots |
| `HelpDialog` (`ui/help_dialog.py`) | Referenz- und Bedienungsanleitung für Tastaturkürzel |