...
| Файл журнала | Путь | Цель |
|---|---|---|
| **Журнал приложения** | `%TEMP%\fastprompter.log` | События активного сеанса, предупреждения, информационные журналы (чередующиеся, максимум 1 МБ, 2 резервные копии). |
| **Журнал сбоев** | `%TEMP%\fastprompter_crash.log` | Подробное отслеживание необработанных критических исключений с помощью sys.excepthook. |
| **Журнал набора тестов** | `%TEMP%\fastprompter-tests.log` | Временный журнал, изолированный от выполнения Pytest. |
