...
**Сворачиваемые изображения:** `![alt](url)` → компактная кнопка 150px. Ctrl+клик открывает, Ctrl+правый клик открывает папку. Двойной клик по пилюле переименовывает файл на диске и ссылку вместе (один шаг undo).
