...
|ログファイル |パス |目的 |
|---|---|---|
| **アプリケーション ログ** | `%TEMP%\fastprompter.log` |アクティブなセッション イベント、警告、情報ログ (ローテーション、最大 1MB、2 つのバックアップ)。 |
| **クラッシュ ログ** | `%TEMP%\fastprompter_crash.log` | 「sys.Excepthook」を介した、未処理の重大な例外の詳細なトレースバック。 |
| **テスト スイートのログ** | `%TEMP%\fastprompter-tests.log` | Pytest 実行の実行に分離された一時ログ。 |
