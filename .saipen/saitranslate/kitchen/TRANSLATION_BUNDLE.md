...
- 14.08.26 [v0.8.34–37 + T-788/T-789] ee prepare run: 5 new `tr()` keys (backup-validation diagnostics `Backup failed validation and was removed:\n{}` and `Restore refused — your current database was left untouched:\n{}` from T-789, `invalid filename` from T-788, and the 2 help-dialog feature strings `queue prompts from a silo…` / `up to 100 tabs…`) added to the 4 Core-owned locales (1020 -> 1025 keys each; EN source, RU/EST/DED hand-translated). Stored `coverage_pct` in the 29 subSaipen-owned locales corrected to the computed 95.2 (was 95.7). Module regeneration deferred to collect (`eee`) — the json↔module gate shows the expected 5-key pending delta in en/ru/est/ded (E-1367 precedent). Validator: 0 missing from en.json; warnings = documented backlog. 29-locale gap + docs lag unchanged (E-1358).
